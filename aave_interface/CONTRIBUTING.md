...
Development builds are run with a watcher and an open server listening on `localhost:3006`.
...
You can also serve up the static site assets with the following command, which can be viewable on `localhost:3006`.
...
**NOTE:** _Always double-check the selected network in your wallet provider to make sure transactions are executed only on the fork network_.
