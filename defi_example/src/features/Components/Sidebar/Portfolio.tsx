...
const amountToNumberFormat = (
  ...
  return Number(ethers.utils.formatUnits(balance, decimals ?? 0)).toFixed(
    fix,
  );
};
...
export const Portfolio = ({ tokens }: ITokenList) => {
  ...
  return (
    <div className="flex flex-col gap-4 w-full h-full overflow-y-hidden">
      <div className="flex space-x-4 font-bold">
        <span
          className={`cursor-pointer ${tabSelected === Tabs.Tokens ? "text-white" : "text-[#98A1C0]"}`}
          onClick={() => setTabSelected(Tabs.Tokens)}
        >
          Tokens
        </span>
        <span
          className={`cursor-pointer ${tabSelected === Tabs.Nfts ? "text-white" : "text-[#98A1C0]"}`}
          onClick={() => setTabSelected(Tabs.Nfts)}
        >
          NFTs
        </span>
        <span
          className={`cursor-pointer ${tabSelected === Tabs.Pools ? "text-white" : "text-[#98A1C0]"}`}
          onClick={() => setTabSelected(Tabs.Pools)}
        >
          Pools
        </span>
        <span
          className={`cursor-pointer ${tabSelected === Tabs.Activity ? "text-white" : "text-[#98A1C0]"}`}
      ...
      <div className="flex flex-col gap-2 h-auto overflow-y-auto">
        {tabSelected === Tabs.Tokens && tokens.map((token, index) => {
