...
const apiClient = createSdk({
  webhookSecret: env.COMPILOT_WEBHOOK_SECRET_DEFI_OFFCHAIN_ZKP,
  apiKey: env.COMPILOT_API_KEY_DEFI_OFFCHAIN_ZKP,
});
...
export default async function handler(
  ...
  try {
    ...
    const challengeRes = await apiClient.createWeb3Challenge({
      workflowId: env.COMPILOT_WORKFLOW_ID_DEFI_OFFCHAIN_ZKP,
      ...params,
    });
