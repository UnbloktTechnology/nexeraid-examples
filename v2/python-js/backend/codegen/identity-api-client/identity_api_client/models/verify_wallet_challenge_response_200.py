from typing import Any, Dict, List, Type, TypeVar, Union

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="VerifyWalletChallengeResponse200")


@_attrs_define
class VerifyWalletChallengeResponse200:
    """
    Attributes:
        token (str):
        external_customer_id (Union[Unset, str]):
    """

    token: str
    external_customer_id: Union[Unset, str] = UNSET
    additional_properties: Dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> Dict[str, Any]:
        token = self.token

        external_customer_id = self.external_customer_id

        field_dict: Dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "token": token,
            }
        )
        if external_customer_id is not UNSET:
            field_dict["externalCustomerId"] = external_customer_id

        return field_dict

    @classmethod
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        d = src_dict.copy()
        token = d.pop("token")

        external_customer_id = d.pop("externalCustomerId", UNSET)

        verify_wallet_challenge_response_200 = cls(
            token=token,
            external_customer_id=external_customer_id,
        )

        verify_wallet_challenge_response_200.additional_properties = d
        return verify_wallet_challenge_response_200

    @property
    def additional_keys(self) -> List[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
